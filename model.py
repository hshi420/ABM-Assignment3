...
class SegAgent(Agent):
    def __init__(self, pos, model, agent_type):  # agents and their characteristics
        ...
        self.wealth = int(random.uniform(0, 1) + 0.5) # poor or rich
        self.similar = 0  # agent-specific measures of neighbor similarity
        self.similar0 = 0
        self.similar1 = 0
        self.similar_wealth = 0
        self.similar_poor = 0
        self.similar_rich = 0
    ...
    def step(self):
        ...
        if self.type == 0:
            if self.similar < 8 * self.model.intolerance:  
                ...
                if self.similar_wealth < 8 * self.model.wealth_intolerance: # move again if unsatisfied in another dimension
                    self.model.grid.move_to_empty(self)
            elif self.similar > 8 * self.model.intolerance and self.similar_wealth > 8 * self.model.wealth_intolerance:
                self.model.happy += 1
                self.model.happy0 += 1
...
class SegModel(Model):
    ...
    def __init__(self, width, height, num_agents, minority_pc, intolerance, wealth_intolerance):
        self.num_agents = num_agents  # we're allowing these values to be set at each run
        self.minority_pc = minority_pc
        self.intolerance = intolerance
        self.wealth_intolerance = wealth_intolerance
        self.width = width
        self.height = height
        ...
        self.datacollector = DataCollector(
            model_reporters={"Avg pct similar neighbors": lambda m: m.pct_neighbors,
                             "Intolerance": lambda m: m.intolerance,
                             "wealth_intolerance": lambda m: m.wealth_intolerance},
